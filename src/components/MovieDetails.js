        ...
        <>
          ...
          <header>
            ...
            <div className="details-overview">
              <h2>{Title} <span style={{ fontWeight: 400, fontSize: "1.6rem", color: "#adb5bd" }}>({Year})</span></h2>
              <p><span>📆</span><span>{Released}</span></p>
              <p><span>⏱</span><span>{Runtime}{Rated && Rated !== "N/A" ? ` · ${Rated}` : ""}</span></p>
              {Language && Language !== "N/A" && (
                <p><span>🌐</span><span>{Language}</span></p>
              )}
              {Awards && Awards !== "N/A" && (
                <p><span>🏆</span><span>{Awards}</span></p>
              )}
              <p><em>{Genre}</em></p>
            </div>
          </header>
          ...
          <section>
            <div className="rating">
              ...
              {isWatched ? (
                ...
                <button
                  className="btn-add"
                  onClick={() => onAddToWatched({ ...movie, userRating })}
                >
                  + Add to Watched
                </button>
              )}
              ...
              <div style={{ display: "flex", gap: "2rem", flexWrap: "wrap" }}>
                {Ratings.map((r) => (
                  <p key={r.Source}>
                    ...
                    <span>{sourceLabel[r.Source] ?? r.Source}: <strong>{r.Value}</strong></span>
                  </p>
                ))}
              </div>
            )}
            ...
            {imdbVotes && (
              <p style={{ color: "#adb5bd", fontSize: "1.3rem" }}>
            ...
            {Writer && Writer !== "N/A" && (
              <p><span>✍️</span><span>Writer: {Writer}</span></p>
            )}
            <p><span>🎭</span><span>Starring: {Actors}</span></p>
            {Country && Country !== "N/A" && (
              <p><span>🌍</span><span>{Country}</span></p>
            )}
          </section>
        </>
      )}
